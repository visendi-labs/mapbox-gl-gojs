...
- There's a bunch of examples to try out - not only by running Go locally but also live "serverside" examples running on the docs page. 
    - Some adaptations have been made to the Go examples in order to make them runnable here on the docs page.
        - They run in a WASM JS context. E.g. Go code that open files can't be WASM-ed, so `//go:embed icon.png` has been used.
	- Some serverside interactivity examples are also running actual, but mocked, HTMX (https://htmx.org) calls to the WASM browser Go "fake" backend. Essentially the same thing as if you spun up the example webserver+web client locally with Go.
